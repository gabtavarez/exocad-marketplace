...
const fallbackOrders: DashboardOrder[] = [
  ...
  { id: 'Caso 24088', patient: 'L. Ferreira', work: 'Faceta · 11, 21', status: 'Em Produção', date: 'Ontem, 16:20', price: 'R$ 620,00' },
...
function StatusBadge({ status }: { status: string }) {
  const tone = status === 'Concluído' ? 'success' : status === 'Em Revisão' ? 'review' : status === 'Em Produção' ? 'drawing' : 'waiting'
...
function ReviewWorkspace({ onBack, role }: { onBack: () => void; role: 'Dentista' | 'Cadista' }) {
  ...
  const [caseStatus, setCaseStatus] = useState<OrderStatus>('IN_REVIEW')
  const [revisionFeedback, setRevisionFeedback] = useState('')
  const hasDeliveryStl = deliveryFiles.some((file) => file.file.name.toLowerCase().endsWith('.stl') && file.status === 'uploaded') || cadAttachments.some((file) => file.fileName.toLowerCase().endsWith('.stl'))

  const runTransition = async (action: () => Promise<OrderResponse>, successMessage: string) => {
    try {
      const updated = await action()
      setCaseStatus(updated.status)
      setDeliveryStatus(successMessage)
    } catch {
      setDeliveryStatus('Não foi possível atualizar o status do caso agora.')
    }
  }
  ...
  return <div className="page-content workspace-page"><div className="breadcrumb"><button onClick={onBack}>Meus Casos</button><ArrowRight /><span>Caso 24091 · Detalhes do Caso</span></div><div className="workspace-heading"><div><div className="eyebrow">Caso 24091 · {mapStatus(caseStatus).toUpperCase()}</div><h1>Detalhes do Caso · Coroa monolítica · Dente 16</h1><p>Paciente: M. Andrade <span className="heading-separator">·</span> Cadista: Rafael Souza</p></div><StatusBadge status={mapStatus(caseStatus)} /></div><CaseTimeline status={caseStatus} /><div className="workspace-grid"><section className="viewer-panel panel"><Clinical3DViewer attachments={viewerAttachments} /></section><aside className="review-sidebar panel"><div className="review-header"><div><h2>Arquivos do caso</h2><p>Entrada clínica e entrega CAD</p></div><span className="online"><Wifi /> Online</span></div><div className="attachment-tabs"><section><div className="section-heading compact"><div className="section-number">01</div><div><h2>Arquivos Clínicos do Caso</h2><p>Enviados pelo dentista</p></div></div><div className="file-checklist">{clinicalAttachments.map((attachment) => <div className="file-row" key={attachment.id}><div className="file-status checked"><FileArchive /></div><div><strong>{attachment.fileName}</strong><span>{formatFileSize(attachment.size)} · {attachment.stage}</span></div><button className="attach-button" onClick={() => setViewerAttachments([attachment])}><ZoomIn />Inspecionar 3D</button></div>)}</div></section><section><div className="section-heading compact"><div className="section-number">02</div><div><h2>Entrega do Design</h2><p>Arquivos finais enviados pelo cadista</p></div></div><div className="file-checklist">{cadAttachments.map((attachment) => <div className="file-row" key={attachment.id}><div className="file-status checked"><FileCheck2 /></div><div><strong>{attachment.fileName}</strong><span>{formatFileSize(attachment.size)}</span></div><button className="attach-button" onClick={() => setViewerAttachments(attachment.fileName.endsWith('.html') ? [attachment] : [attachment, ...clinicalAttachments.filter((item) => item.fileName.endsWith('.stl'))])}>{attachment.fileName.endsWith('.html') ? 'Webview 3D' : 'Visualizar 3D'}</button><button className="remove-file" aria-label="Baixar arquivo"><Download /></button></div>)}</div>{role === 'Cadista' && <div className="delivery-upload"><CaseFileUpload files={deliveryFiles} onFilesChange={setDeliveryFiles} stage="CAD_DELIVERY" acceptedExtensions={cadDeliveryExtensions} helperText=".stl, .constructioninfo ou .html · sem pastas brutas ou zips" />{deliveryStatus && <p className="escrow-note">{deliveryStatus}</p>}<button className="primary-button full" onClick={uploadDelivery} disabled={deliveryFiles.length === 0}>Enviar entrega CAD</button></div>}{role === 'Dentista' && <label className="field revision-field"><span>Observações para ajuste</span><textarea value={revisionFeedback} onChange={(event) => setRevisionFeedback(event.target.value)} placeholder="Descreva o ajuste clínico necessário..." /></label>}</section></div><div className="review-actions">{role === 'Cadista' && <><button className="request-button" onClick={() => runTransition(() => acceptOrder(1), 'Caso assumido pelo cadista.')} disabled={caseStatus !== 'OPEN'}><CheckCircle2 />Assumir Caso</button><button className="approve-button" onClick={() => runTransition(() => submitOrderDelivery(1), 'Caso enviado para aprovação.')} disabled={caseStatus !== 'IN_PROGRESS' || !hasDeliveryStl}><UploadCloud />Enviar para Aprovação</button></>}{role === 'Dentista' && <><button className="approve-button" onClick={() => runTransition(() => approveOrder(1), 'Design aprovado.')} disabled={caseStatus !== 'IN_REVIEW' || !hasDeliveryStl}><CheckCircle2 />Aprovar Design</button><button className="request-button" onClick={() => runTransition(() => requestOrderRevision(1, revisionFeedback), 'Ajuste solicitado ao cadista.')} disabled={caseStatus !== 'IN_REVIEW' || revisionFeedback.trim().length === 0}><MessageCircle />Pedir Ajuste</button></>}</div></aside></div></div>
}
...
export default function OdontoMarketplace() {
  ...
  const screen = active === 'Novo Caso'
    ...
    : active === 'Meus Casos'
      ? <ReviewWorkspace onBack={() => setActive('Visão geral')} role={role} />
      : role === 'Cadista'
        ? <DesignerBoard />
